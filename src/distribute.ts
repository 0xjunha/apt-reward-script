...
const APT_AMOUNT = 100_000; // in octas (1 APT = 100_000_000 octas)
...
async function main() {
    ...
    const sender: Account = Account.fromPrivateKey({privateKey: new Ed25519PrivateKey(admin_pk)});
    const sender_balance = await aptos.getAccountAPTAmount({accountAddress: sender.accountAddress});
    console.log(`sender (${sender.accountAddress}) APT balance: ${sender_balance} octas`)

    // APT reward recipients addresses
    const recipient_addresses: AccountAddress[] = [
        AccountAddress.fromString("0x1f8c8ca3cdce70a9bf9cadcb8976ff28b26935a456d5e2004730809e85070f62"),
        AccountAddress.fromString("0xc515ea4268a4c22789b7c8fab2051d9cd828487795206a96c00f10dac8fe7d19"),
        AccountAddress.fromString("0xff9ec3d2bce27109103aaa613037c2130a0cb76be923d6e31fa91b55781e9721"),
        AccountAddress.fromString("0x794d6c3bdb7949dde2860ac1896a0682c22aff937572151e7f18c3b7a17c6763"),
        AccountAddress.fromString("0x035d6305afe22a01b6a38308eb69761a58fec0a386bbdd00c3a57a3b6ac5c998"),
    ];
    ...
    // batch transfer
    console.log(`transferring ${APT_AMOUNT} octas to ${recipient_addresses.length} addresses...`)
    ...
    aptos.transaction.batch.on(TransactionWorkerEventsEnum.TransactionSent, async (data) => {
      ...
      aptos.transaction.batch.on(TransactionWorkerEventsEnum.ExecutionFinish, async (data) => {
        ...
        const senderAccountData = await aptos.getAccountInfo({ accountAddress: sender.accountAddress });
        console.log(`sender account's sequence number is ${senderAccountData.sequence_number}`);
